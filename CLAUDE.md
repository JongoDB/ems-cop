...
EMS-COP is a **dual-enclave** platform for planning, approving, executing, and supervising **offensive (red team) and defensive (blue team / SOC) cyberspace operations**. Unified workspace with approval chains, audit trails, echelon-appropriate visibility, and cross-domain data controls.

- **Low Network** (Internet-facing): Tactical ops, C2 traffic, endpoint telemetry, field operator access
- **High Network** (Protected): Strategic planning, analysis, sensitive findings, command authority, consolidated COP
- **CTI** (Apache NiFi): Bridges enclaves with provenance and policy-based data flow control

Current version: **v0.16.0** (M12 complete). Next: M13 (DCO/SOC), M14 (Deployment Portability).

## Services (10 per enclave)

Same binaries, different config via `ENCLAVE=low|high` env var.

| Service | Lang | Port | Route Prefix |
|---------|------|------|-------------|
| auth | Go | 3001 | `/api/v1/auth` |
| workflow-engine | Go | 3002 | `/api/v1/workflows` |
| ticket | Node/TS | 3003 | `/api/v1/tickets` |
| dashboard | Node/TS | 3004 | `/api/v1/dashboards` |
| c2-gateway | Go | 3005 | `/api/v1/c2` |
| audit | Go | 3006 | `/api/v1/audit` |
| notification | Node/TS | 3007 | `/api/v1/notifications` |
| endpoint | Go | 3008 | `/api/v1/endpoints` |
| ws-relay | Node/TS | 3009 | `/ws` |
| frontend | React/TS | 80 | `/` |

Also: **cti-relay** (Go, 3010, `/api/v1/cti`) — cross-domain bridge service.

## Design Principles

1. **User-sovereign customization** — Roles, workflows, dashboards are ALL runtime-configurable
2. **Audit everything** — Every action → NATS → ClickHouse. Append-only, hash-chained
3. **Classification-aware** — UNCLASS/CUI/SECRET on all entities. No-downgrade. SECRET never crosses CTI
4. **C2-agnostic** — `C2Provider` interface: Sliver, Mythic, Havoc, or custom. Per-operation selection
5. **Enclave-portable** — Same binaries both sides, behavior driven by `ENCLAVE` env var
6. **Deploy anywhere** — Docker Compose (dev), Helm (K8s), Ansible/Terraform (bare metal)

## Testing

**1,251+ tests** across all services + frontend:
- **Go**: `go test ./...` — table-driven tests (auth, audit, workflow-engine, endpoint, c2-gateway, cti-relay)
- **Node**: Jest (ticket, dashboard, notification, ws-relay)
- **Frontend**: Vitest + RTL
- 28 tests skip without live infra (by design)

## Critical Rules

- SECRET data must **NEVER** be on the low side or cross the CTI
- Classification can only be **upgraded**, never downgraded
- API errors: `{ "error": { "code": "NOT_FOUND", "message": "..." } }`, `X-Classification` header on all responses
- ClickHouse: `MergeTree` only (NOT `ReplacingMergeTree`), TTL uses `toDateTime(timestamp)`
- Traefik uses **file-based provider** (not Docker). Routes in `infra/traefik/dynamic*.yml`
- CTI relay requires `CTI_API_TOKEN` env var; connects ONLY to `cti-net`
- Seed passwords are bcrypt of `changeme` — NEVER use in production
- `endpoint-net` is isolated — only C2 gateway and Sliver bridge both networks

## Reference Docs

For detailed information, agents should read these files on demand:
- **Architecture details** (topology, CTI, auth model, DCO, data stores, env vars, NATS topics): `docs/ARCHITECTURE.md`
- **Milestone history** (M1-M12 details, security findings, known limitations): `docs/MILESTONES.md`
- **Key files lookup** (service entry points, infra files, frontend components): `docs/KEY_FILES.md`
- **Full requirements & user stories**: `docs/SYSTEM_DESIGN.md`
- **Design plans**: `docs/plans/`

## Development Environment

Runs inside the [Trail of Bits Claude Code devcontainer](https://github.com/trailofbits/claude-code-devcontainer).
`bypassPermissions` enabled — agents run unrestricted (sandboxed by Docker).

**Workflow**: `devc .` → `devc shell` → `claude`
**Tools**: gh CLI, claude CLI, Node 22 (fnm), ripgrep, fd, tmux, fzf, delta, ast-grep
